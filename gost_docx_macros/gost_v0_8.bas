Sub GOST()
    ...
    ' Find the starting point of the third page
    Dim startRange As Range
    Dim startOfThirdPage As Long
    
    Set startRange = doc.Range(0, 0)
    startRange.GoTo What:=wdGoToPage, Which:=wdGoToAbsolute, Count:=3
    startOfThirdPage = startRange.Start
    ...
    With doc.Styles(wdStyleHeading4).Font
        ...
        .Bold = True
    End With
    doc.Styles(wdStyleHeading4).ParagraphFormat.Alignment = wdAlignParagraphLeft
    doc.Styles(wdStyleHeading4).ParagraphFormat.SpaceAfter = 12
    
    With doc.Styles(wdStyleHeading5).Font
        .Name = "Times New Roman"
        .Size = 14
        .Bold = True
    End With
    doc.Styles(wdStyleHeading5).ParagraphFormat.Alignment = wdAlignParagraphLeft
    doc.Styles(wdStyleHeading5).ParagraphFormat.SpaceAfter = 12

    With doc.Styles(wdStyleHeading6).Font
        .Name = "Times New Roman"
        .Size = 14
        .Bold = True
    End With
    doc.Styles(wdStyleHeading6).ParagraphFormat.Alignment = wdAlignParagraphLeft
    doc.Styles(wdStyleHeading6).ParagraphFormat.SpaceAfter = 12

    With doc.Styles(wdStyleHeading7).Font
        .Name = "Times New Roman"
        .Size = 14
        .Bold = True
    End With
    doc.Styles(wdStyleHeading7).ParagraphFormat.Alignment = wdAlignParagraphLeft
    doc.Styles(wdStyleHeading7).ParagraphFormat.SpaceAfter = 12

    ' Force update of all Heading 3 text - only for pages 3 onward
    Dim para As Paragraph
    For Each para In doc.Paragraphs
        ' Check if paragraph starts on or after third page
        If para.Range.Start >= startOfThirdPage Then
    ...
    ' Ensure first line indent for all normal paragraphs - only for pages 3 onward
    For Each para In doc.Paragraphs
        ' Check if paragraph starts on or after third page
        If para.Range.Start >= startOfThirdPage Then
            If para.Style = doc.Styles(wdStyleNormal) Then
                para.Format.FirstLineIndent = CentimetersToPoints(1.25)
            End If
        End If
    Next para
